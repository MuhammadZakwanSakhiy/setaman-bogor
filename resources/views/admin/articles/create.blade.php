...
@section('title', 'Tulis Artikel | Admin Setaman Bogor')

@section('content')
<div class="mb-8 border-b border-gray-200 pb-4 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-gray-900 mb-1">TULIS ARTIKEL BARU</h2>
        <p class="text-sm text-gray-500">Buat artikel edukasi atau berita baru untuk pelanggan.</p>
    </div>
    ...
    <form action="{{ route('articles.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-1 focus:ring-brand focus:border-brand" placeholder="Cth: Cara Merawat Monstera Deliciosa">
            </div>
            ...
            <div class="border border-gray-300 rounded-md p-4 bg-gray-50">
                <label for="image" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Foto Utama Artikel</label>
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="10" required class="w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-1 focus:ring-brand focus:border-brand" placeholder="Tuliskan isi artikel Anda di sini...">{{ old('content') }}</textarea>
            </div>

            <!-- Status Toggles -->
            <div class="pt-4 border-t border-gray-200">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_published" value="1" {{ old('is_published', true) ? 'checked' : '' }} class="w-5 h-5 text-brand rounded focus:ring-brand">
        ...
        <div class="pt-6 border-t border-gray-200 flex justify-end gap-4">
            ...
            <button type="submit" class="px-6 py-3 bg-black text-white rounded-md text-sm font-bold uppercase tracking-wider hover:bg-gray-800 transition">Simpan Artikel</button>
        </div>
    </form>
</div>
@endsection
