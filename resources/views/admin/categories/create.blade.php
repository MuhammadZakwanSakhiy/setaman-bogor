...
@section('title', 'Tambah Kategori | Admin Setaman Bogor')

@section('content')
<div class="mb-8 border-b border-gray-200 pb-4 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-gray-900 mb-1">TAMBAH KATEGORI</h2>
        <p class="text-sm text-gray-500">Buat kategori baru untuk produk.</p>
    </div>
...
<div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6 max-w-2xl">
    <form action="{{ route('categories.store') }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="name" value="{{ old('name') }}" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-brand @error('name') border-red-500 @enderror" required>
            @error('name')
        ...
        <div class="mb-6">
            ...
            <textarea name="description" rows="3" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-brand">{{ old('description') }}</textarea>
        </div>
